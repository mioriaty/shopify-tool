...
import { v4 } from 'uuid';
import { ServerSmartBarService, SmartBarServiceModel } from './types/server';
...
const statisticDetails = createState<ServerSmartBarService>({
  status: 'success',
  data: {
    items: [],
    maxPages: 0,
...
export async function getStatisticDetail(id: string): Promise<SmartBarServiceModel> {
  await delay(400);
  return {
    ...(statisticDetails.getState().data.items.find(item => item.id === id) as SmartBarServiceModel),
