...
const wiloke_end_point = process.env.WILOKE_API_ENDPOINT || configureApp.baseUrl;

const axiosConfig = new ConfigureAxios({
  configure: {
    method: 'GET',
    baseURL: wiloke_end_point,
    timeout: configureApp.timeout,
    paramsSerializer: qs.stringify,
  },
  setAccessHeader() {
    return {
      accessToken: store ? store.getState().authPage.auth.loginToken : '',
      'X-ShopName': store ? store.getState().authPage.auth.shopName : '',
    };
  },
  setRefreshToken() {
